...
- **Web interface** - Minimal web frontend for easy monitoring
...
- Управление проверкой HLS потоков
- Просмотр логов в реальном времени
- Отображение результатов проверки
- Доступ к экспортированным данным

### RESTful API

Веб-интерфейс также предоставляет RESTful API для программного доступа:
...
curl -X POST http://localhost:5000/api/check \
  ...
  -d '{"channelCount": "5", "duration": 3}'

# Получение последних данных
curl http://localhost:5000/api/data/latest
```

Полная документация API доступна в файле [WEB_API.md](WEB_API.md).
...
make run         # Запуск проверки HLS потоков
