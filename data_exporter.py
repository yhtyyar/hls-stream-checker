...
class OptimizedDataExporter:
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as csvfile:
            ...
            for channel in global_stats.channels:
                if channel.total_segments > 0:  # Только каналы с данными
                    ...
                    row = {
                        "время_начала_сессии": self.session_start.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
                        "время_окончания_сессии": self.session_end.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        ),
    ...
    def export_global_summary_csv(self, global_stats) -> Path:
        ...
        total_mb = (
            global_stats.total_bytes / (1024 * 1024)
            if global_stats.total_bytes > 0
            else 0
        )
        avg_speed = (
            (total_mb / self.session_duration) if self.session_duration > 0 else 0
        )
    ...
    def export_optimized_json(self, global_stats) -> Path:
        ...
        api_data = {
            "analytics": {
                "overall_analysis": {
                    ...
                    "error_distribution": (
                        global_stats.error_counts
                        if hasattr(global_stats, "error_counts")
                        else {}
                    ),
                }
            },
        ...
        if api_data["channels"]:
            best_channel = max(
                api_data["channels"], key=lambda x: x["stats"]["success_rate"]
            )
            worst_channel = min(
                api_data["channels"], key=lambda x: x["stats"]["success_rate"]
            )
